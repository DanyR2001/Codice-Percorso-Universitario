...
def search(prodotto,listaProduttori):
    for tripla in listaProduttori:
        nome, numero,data = tripla
        prodotto.aggiorna(nome,numero, data)
    String="Il dizionario acquirenti del prodotto {} contiene {} entrate dopo i nuovi inserimenti.\n".format(prodotto.nome,len(prodotto.acquirenti))
    String+="Le entrate del dizionario aggiornato contengono:\n"
    for key,value in prodotto.acquirenti.items():
        String+=key+" "+str(value[0])+" "+str(value[1])+"\n"
    return String



def add_jobs(listaProdotti, listaAcq, jobs):
    for (prodotto, listaVenditori) in zip(listaProdotti,listaAcq):
        jobs.put((prodotto,listaVenditori))

def worker(jobs,results):
    while True:
        prodotto,listaAcq=jobs.get()
        try:
            result = search(prodotto,listaAcq)
            results.put(result)
        finally:
            jobs.task_done()
...
def processaProdotti(prodotti: list[ProdottoSemplice], liste_acquirenti:list, concorrenza:int):
    jobs=multiprocessing.JoinableQueue()
    results=multiprocessing.Queue()
    add_jobs(prodotti,liste_acquirenti, jobs)
